...
class CanopyGrid():
    ...
    def run_timestep(self, doy, dt, Ta, Prec, Rg, Par, VPD, U=2.0, CO2=380.0, Psi=0.0, Rew=1.0, beta=1.0, P=101300.0):
        """
        ...
        Transpi, Efloor, Gc, fW, fQ, Tr0 = self.dry_canopy_et(VPD, Par, Rn, Ta, Ra=Ra, Ras=Ras, CO2=CO2, Psi=Psi, beta=beta, fPheno=fPheno)
        ...
        return PotInf, Trfall, Interc, Evap, ET, Transpi, Efloor, MBE, fW, fQ, ET0
    ...
    def dry_canopy_et(self, D, Qp, AE, Ta, Ra=25.0, Ras=250.0, CO2=380.0, Rew=1.0, Psi=0.0, beta=1.0, fPheno=1.0):
        """
        ...
        g1_beta = self.physpara['g1_beta'] # stomatal conductance sensitivity to soil water potential
        ...
        #fW= np.minimum(1.0, np.maximum(Rew / rw, rwmin))
        #fW = 1.0
        
        # stomatal conductance response to soil water potential [MPa]
        fW = np.exp(g1_beta * Psi)
        #print(fW)
        ...
        # canopy conductance
        Gc = gs * fQ * fW * fCO2 * fPheno
        Gc[np.isnan(Gc)] = eps
        
        """ --- for non water-restricted actual transpiration """
        
        Gc0 = gs * fQ * fCO2 * fPheno
        Tr0 = penman_monteith((1.-tau)*AE, 1e3*D, Ta, Gc0, 1./Ra, units='mm')
        Tr0[Tr0 < 0] = 0.0
