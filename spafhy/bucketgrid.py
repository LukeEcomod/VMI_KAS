...
class BucketGrid(object):
    ...
    def __init__(self, spara, outputs=False):
        ...
        CHANGES:
            3.10.2018 changes intialization completely. Samuli
        """
        ...
        self.vanG_alpha = spara['alpha']    # kPa-1
        self.vanG_n = spara['n']
        self.vanG_Theta_s = spara['poros']  # m3m-3
        self.vanG_Theta_r = spara['wr']     # m3m-3
    ...
    def watbal(self, dt=1.0, rr=0.0, tr=0.0, evap=0.0, retflow=0.0):
        ...
        #K = self.hydrCond()
        Cond = self.hydraulic_conductivity()
        drain = np.minimum(Cond * dt, np.maximum(0.0, (self.Wliq - self.Fc))*self.D)
    ...
    def theta_psi(self):
        ...
        x = np.minimum(self.Wliq, self.vanG_Theta_s)
        x = np.maximum(x, self.vanG_Theta_r)  # checks limits
        
        s = (self.vanG_Theta_s - self.vanG_Theta_r) / ((x - self.vanG_Theta_r) + eps)
        
        Psi = -1 / self.vanG_alpha*(s**(1.0 / m) - 1.0)**(1.0 / n)  # kPa
        Psi[Psi==np.NaN] = 0.0
        Psi = 1e-3*Psi
        #Psi[Psi<-3.0] = -3.0
        return Psi
    ...
    def hydraulic_conductivity(self):
        ...
        # eff sat
        Seff = np.minimum(1.0, (self.Wliq - self.vanG_Theta_r) / (self.vanG_Theta_s - self.vanG_Theta_r))
